...
def backward(grad_fn, grad_of_outputs):
    ...
    grads_pair=grad_fn.apply(grad_of_outputs) 
    lst=[]
    if type(grads_pair) == tensor.Tensor:
        grads_pair = (grads_pair,)
    if grads_pair is None:
        return
    if len(grads_pair) == 2:
        if grads_pair[0] is not None:
            lst.append((grads_pair[0],grad_fn.next_functions[0]))
        if grads_pair[1] is not None:
            lst.append((grads_pair[1],grad_fn.next_functions[1]))
    else:
        lst.append((grads_pair[0],grad_fn.next_functions[0]))
    if len(grad_fn.next_functions) != 0:
        for item in lst:
            grad_of_outputs = item[0]
            grad_fn = item[1]
            if grad_fn is not None:
                backward(grad_fn, grad_of_outputs)
...
class Function:
    ...
    def apply(cls, *args):
        ...
        for arg in args:
            if(type(arg)==tensor.Tensor):
                if arg.is_leaf and arg.requires_grad:
